...
// the cube's subcells, exactly as the CPU original); the truncated-octahedron checkerboard
...
// GPU render state.
static GLuint g_vbo = 0;       // TO vertex positions (selected cells x 24 solid / 96 hollow verts, dynamic)
...
static std::vector<float> g_deformedTO;  // per-frame expanded vertex buffer, uploaded to g_vbo

// Forward declarations (defined after `cube` and the camera globals).
static void rebuildBuffers();
static void updateTOGeometry();
static void drawGPU();
...
void Setup() {
	...
	if (ciclo == 0) {
        ...
        cube.writeSTL_s_truncated_octahedron("/home/mike666/Downloads/mesh_output_truncated_octahedron.stl", "MyTruncatedOctahedron", "checkerboard", ii, kk, jj, g_morphS, g_hollow, g_inset);

        // GPU path: build the TO IBO + size the VBO for the current
...
// path still builds Facets via getCheckerboardFacetsTruncatedOctahedron on demand).
void Draw() {
    // Intentionally empty — the TO mesh is rasterized by drawGPU() below.
}
...
//==============================================================================
// GPU geometry buffers (truncated-octahedron checkerboard lattice).
// rebuildBuffers(): build the static TO IBO + (re)allocate the VBO to the
//   current selection size. Independent of deformation — rebuild only when
//   <ii,jj,kk> or g_hollow changes (g_selectionDirty).
// updateTOGeometry(): expand the (just-deformed) subcells into the flat
//   TO vertex buffer and upload it to the VBO. Called every animated frame.
...
static void rebuildBuffers() {
    ...
    cube.fillCheckerboardIndicesTruncatedOctahedron(ii, kk, jj, indices, g_hollow, g_inset);
    ...
    // VBO size = selectedCount * vertsPerCell * 3 floats. selectedCount derives from
    // the triangle count (44 solid / 144 hollow per cell) — identical to the count
    // fillTruncatedOctahedronVertexLattice will emit (same predicate + active filter).
    // NOTE: unlike the octagonal mesh (126 verts/cell regardless of hollow), the TO
    // VBO is hollow-DEPENDENT: 24 verts/cell solid, 96 verts/cell hollow.
    const size_t trisPerCell   = g_hollow ? 144 : 44;     // TO: 144 hollow / 44 solid tris per cell
    const size_t vertsPerCell   = g_hollow ? 96 : 24;      // TO: 96 hollow / 24 solid verts per cell
    ...
    const size_t floats = selectedCount * vertsPerCell * 3;
    g_deformedTO.assign(floats, 0.0f);   // sized for glBufferSubData; refilled each frame

    if (g_vbo == 0) glGenBuffers(1, &g_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, g_vbo);
    glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)(floats * sizeof(float)),
                 g_deformedTO.data(), GL_DYNAMIC_DRAW);
...
static void updateTOGeometry() {
    cube.fillTruncatedOctahedronVertexLattice(ii, kk, jj, g_deformedTO, g_morphS, g_hollow, g_inset);
    if (g_deformedTO.empty()) return;
    glBindBuffer(GL_ARRAY_BUFFER, g_vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
                    (GLsizeiptr)(g_deformedTO.size() * sizeof(float)),
                    g_deformedTO.data());
    glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
static void parseArgs(int argc, char** argv) {
    ...
    printf("[slow-inv] mode=%s  speed=%.3f  (per-inversion ~%.1fs)  hollow=%s  s=%.2f  inset=%.2f\n",
           g_slowMode ? "SLOW" : "ORIGINAL", g_invSpeed,
           g_slowMode ? 1.0 / (INV_RATE_PER_TICK * 30.0 * g_invSpeed) : 0.0,
           g_hollow ? "ON" : "OFF", g_morphS, g_inset);
}
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow(" JAZ 4D Enhanced Camera U.U  (Truncated Octahedron)");
...
void display()
{
    ...
    // GPU buffers: rebuild the TO IBO + size the VBO when the selection
    ...
    if (!g_animPaused || selectionChanged) updateTOGeometry();
...
void captureSTLSnapshot()
{
    ...
    try {
        // EXACTLY the render's selection (drawGPU rasterizes the same checkerboard).
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        // s changes positions but NOT vertex/triangle counts, so the VBO size is
        // unchanged; g_selectionDirty forces updateTOGeometry to refill it
        // (also works while the animation is paused).
        case '-':
        ...
        // Inset scrub: ',' decrease (thicker border / smaller hole), '.' increase.
        case ',':
        ...
        // => 144 frame tris/cell (hex/square holes) instead of 44 solid. Counts
        // change (24<->96 verts, 44<->144 tris) so the VBO must resize -> dirty.
        case 'o':
        case 'O':
            g_hollow = !g_hollow;
