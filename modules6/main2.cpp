...
void Setup() {
	...
	if (ciclo == 0) {
		...
		/*0*/
		cube0 = Cube(1.0, origen);
		slash = ModuleSpaces(10.0 * (I + J + K), origen, cube0[0]);
        	        ...
        	        for (int j = 0; j < n; j++)
        	                for (int k = 0; k < n; k++)
        	                        for (int l = 0; l < 1; l++) {
        	                                ...
        	                                int ttt = ternaryCat(i, j, k, l);
        	                                if (ttt == 1) {
        	                                        Vector3D a0 = Vector3D(i * ds, j * ds, k * ds);
        	                                        a0 = a0 - 0.5*(n-1) * Vector3D(ds, ds, ds);
        	                                        menger[cont] = Cube(rs, a0);
							cont++;
        	                                }
        	                        }
...
void Draw() {
	...
	if (ciclo > 0) {
		...
		for (int i = 0; i < 329; i++) {
			slash = ModuleSpaces(
					spherical(10, rot*M_PI, rotAxe*M_PI), 
					spherical(count3, rot*M_PI, rotAxe*M_PI), menger[i][0]);	
			if (slash.getCut() != 0) {
				drawFacetBox(slash[0], 255,  0, 0);
				drawFacetBoxTrans(slash[1],   0, 0, 255, 0.85);
			} else {
			      drawFacetBox(menger[i][0], 255,  0, 255);
			}
			slash.restart(); 
		}
