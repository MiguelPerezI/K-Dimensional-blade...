...
static void parseArgs(int argc, char** argv) {
    ...
    printf("[slow-inv] mode=%s  speed=%.3f  (per-inversion ~%.1fs)\n",
...
static void captureFrameSTL(int frameIndex) {
    ...
    try {
        FacetBox sel = cube.getCheckerboardFacets(ii, kk, jj);  // matches Draw()/captureSTLSnapshot()
        writeFacetBox(sel, path, "MyCube");
