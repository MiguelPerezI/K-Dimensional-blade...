...
void render(int count, int core) {
	...
	//QUATERNIONS
		...
		double sliceThickness = pov.piecewiseNumber(2, 0.0, count, 160);//0.05;
...
//		space.closeSlice(sliceThickness);
		...
		u0 = pov.piecewiseVector(D.getQ(17), D.getQ(18), 50, 100);
		ModuleSpace space4 = ModuleSpace(u0, (1/gold) * u0, space2.getFacetBox0());

                space4.openSlice(sliceThickness);

		for (int i = 0; i < space4.getN0(); i++) {

                        Quaternion Aa = space4.getFacet0(i).getA();
                        Quaternion Ca = space4.getFacet0(i).getC();
                        Quaternion Ba = space4.getFacet0(i).getB();

                        pov.drawFacet(Aa, Ca, Ba, 2, 5, 1, 0.0);
                        stl.facetSTL(Aa.V(), Ca.V(), Ba.V());

                }

                for (int i = 0; i < space4.getN1(); i++) {

                        Quaternion Aa = space4.getFacet1(i).getA();
                        Quaternion Ca = space4.getFacet1(i).getC();
                        Quaternion Ba = space4.getFacet1(i).getB();
