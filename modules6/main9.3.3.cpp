...
struct SpawnRecord { FacetBox mesh; Vector3D center; double radius; int view; };
...
void Draw() {
	...
	if (ciclo > 0) {
        ...
        // Each spawn gets its own hue so siblings are distinguishable from the
        // rainbow original; alpha 0.7 so the source still reads through.
        for (size_t s = 0; s < g_spawned.size(); ++s) {
            const FacetBox& m = g_spawned[s].mesh;
            size_t n = m.size();
            float hue = fmodf(190.0f + 37.0f * (float)s, 360.0f);
            Color c = hsv2rgb(hue, 0.55f, 1.0f);
            int R = int(c.r * 255), G = int(c.g * 255), B = int(c.b * 255);
            for (size_t i = 0; i < n; ++i)
                drawFacet(m[i], R, G, B, 0.7f);
        }
...
// ENTER: spawn a spherically-inverted copy of the active mesh through the sphere
// defined by the selected vertices, glued to those vertices. Pushed onto the
// spawn history (Ctrl+Z undoes, Ctrl+Y redoes).
void spawnInversion() {
    ...
    const FacetBox& active = currentActiveBox();
    if (active.size() == 0) {
        std::cout << "Spawn: no mesh loaded.\n";
        return;
    }

    // Collect selected vertex positions in the active-view space. Selection
    // indices map across views (pointwise images), so this matches the screen.
    std::vector<Vector3D> verts;
    verts.reserve(g_selectedVerts.size());
    for (int idx : g_selectedVerts) {
        const VertRef& r = g_uniqueVerts.at(idx);
        verts.push_back(active[r.facetIdx][r.cornerIdx]);
    }
    ...
    g_spawned.push_back({child, center, radius, g_view});
    g_undone.clear();                                 // a new action clears redo

    std::cout << "Spawned inverted mesh #" << g_spawned.size()
              << " | center " << center << "radius " << radius
              << " | phi*rho = " << (radius / g_phi)
              << " | from " << verts.size() << " verts (view " << g_view << ")\n";
    glutPostRedisplay();
}

// Ctrl+Z: revert the most recent spawn (move it onto the redo stack).
void undoSpawn() {
    if (g_spawned.empty()) {
    ...
    g_spawned.pop_back();
    std::cout << "Undid spawn -> " << g_spawned.size()
              << " visible, " << g_undone.size() << " on redo stack.\n";
    glutPostRedisplay();
}

// Ctrl+Y: re-apply the most recently undone spawn.
void redoSpawn() {
    if (g_undone.empty()) {
    ...
    g_undone.pop_back();
    std::cout << "Redid spawn -> " << g_spawned.size()
              << " visible, " << g_undone.size() << " on redo stack.\n";
    glutPostRedisplay();
}
...
void drawHUD() {
    ...
    const char* lines[] = {
        ...
        "[C]: Clear selection",
        "Enter: Spawn inverted mesh",
        "Ctrl+Z: Undo | Ctrl+Y: Redo spawn",
...
void keyboard(unsigned char key, int /*x*/, int /*y*/) {
    switch (key) {
        ...
        case 'c': case 'C':                           // clear the selection
            ...
            g_selectedVerts.clear();
            break;
        case 13: case 10: spawnInversion(); break;    // ENTER spawns inverted mesh
